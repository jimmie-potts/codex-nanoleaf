...
class ControllerWorkerTest(unittest.TestCase):
    ...
    def test_machine_quiet_records_actual_transmission(self):
        self.command('Quiet')
        self.run_worker([(1001,lambda:modes.set_mode(self.directory,'free',launch=lambda _:None,now=self.clock.now))])
    ...
    def test_same_free_noop_never_sends(self):
        modes.set_mode(self.directory,'free',launch=lambda _:None,now=self.clock.now);self.run_worker()
    ...
    def test_revoked_unsent_mode_cannot_run_as_legacy_work(self):
        self.command('Quiet');server.revoke(self.directory,'client')
        self.run_worker()
        self.assertEqual(self.device.calls,[])
    ...
    def test_revocation_during_observe_cannot_fall_back_to_legacy_send(self):
        ...
        with patch.object(self.device,'request',request):self.run_worker()
    ...
    def test_expiry_during_unread_cannot_fall_back_to_legacy_send(self):
        ...
        def unread():
            ...
            return self.unread
        b.run_worker(self.directory,sleep=self.clock.sleep,now=self.clock.now,read_unread=unread,request=self.device.request)
        self.assertEqual([call for call in self.device.calls if call[1]=='PUT'],[])
        self.assertEqual(self.last()['priorEffects'],'none')
    ...
    def test_disable_during_observe_cannot_fall_back_to_legacy_send(self):
        ...
        def request(config,method,endpoint='',payload=None):
            ...
            if method=='GET' and not disabled:
                disabled=True
                server.command(['controller-disable','--state-dir',str(self.directory)])
            return original(config,method,endpoint,payload)
        original=self.device.request
        with patch.object(self.device,'request',request):self.run_worker()
