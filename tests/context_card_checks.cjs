...
module.exports = async function(page, root) {
  ...
  try {
    ...
    for (const selector of ['#reservation', '#swap', '#locateHint']) assert.equal(await visible(selector), false, `${selector} is hidden in Classic`);
    assert.equal(await page.getByRole('combobox', {name: 'Task project override'}).count(), 0, 'The map offers no task override');
    ...
    await options.close(page);
    await wallLine(lines[0]).click(); await refresh(); await settle();
    assert.equal(await visible('#selectionHint'), false, 'The reservation row replaces the hint');
    for (const selector of ['#reservation', '#swap']) assert.equal(await visible(selector), true, `${selector} shows in Project layout`);
    assert.equal(await page.locator('#reservation label').textContent(), 'Reserved for');
    assert.equal(await page.locator('#assignProject').inputValue(), '', 'An unreserved Line reads Shared pool');
    assert.equal(await page.getByRole('combobox', {name: 'Task project override'}).count(), 0, 'Project layout offers no task override either');
    assert.equal(await page.locator('#contextCard button, #contextCard select').count(), 3, 'Project layout adds only the reservation select and Swap halves');
    await wallLine(lines[3]).click();
    assert.equal(await page.locator('#assignProject').inputValue(), snapshot.projects[0].id, 'A reserved Line shows its project');
    assert.match(await page.locator('#taskDetail').textContent(), /No task on this Line/);
    await wallLine(lines[0]).click({modifiers: ['Control']});
    assert.equal(await page.locator('#assignProject').inputValue(), 'mixed', 'Lines with different reservations read Several reservations');
    assert.equal(await page.locator('#assignProject option[value="mixed"]').evaluate(node => node.disabled), true);
    await wallLine(lines[0]).click();
    await page.locator('#assignProject').selectOption(snapshot.projects[1].id);
    await page.waitForFunction(id => state.lines.find(line => line.id === id)?.project, lines[0].id);
    assert.deepEqual(assigned.at(-1), {lines: {[lines[0].id]: {project: snapshot.projects[1].id}}}, 'Choosing a project sends the existing assignment request');
    assert.equal(await page.locator('#assignProject').inputValue(), snapshot.projects[1].id);
    await wallLine(lines[0]).click(); await wallLine(lines[2]).click({modifiers: ['Control']});
    await page.locator('#assignProject').selectOption('');
    await page.waitForFunction(id => !state.lines.find(line => line.id === id)?.project, lines[0].id);
    assert.deepEqual(assigned.at(-1), {lines: {[lines[0].id]: {project: null}, [lines[2].id]: {project: null}}}, 'Several selected Lines change together');
    await wallLine(lines[0]).click();
    ...
    await options.close(page);
    for (const selector of ['#reservation', '#swap']) assert.equal(await visible(selector), false, `${selector} hides again in Classic`);
    assert.deepEqual(writes, ['/api/settings', '/api/assign', '/api/assign', '/api/settings'], 'Only layout changes and reservations wrote');
