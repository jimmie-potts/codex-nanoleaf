...
def recount(value):
    ...
    for session in sessions:
        ...
        for child in sessions:
            if child['parent']['status'] != 'known' or child['parent']['identity'] != session['identity']: continue
...
class ChildSessionTest(SelectionTest):
    ...
    def test_notice_lifecycle_follows_documented_count_and_allocation(self):
        ...
        # A new turn: the owner has cleared the earlier notice for this consumer.
        self.root['turn'] = {'status': 'known', 'id': 'next'}; self.root['activity'] = 'active'; self.advance(1004)
        self.assertEqual(self.tasks(1004), {self.key: ('working', 100, 'current')})
        ...
        self.advance(1005)
        self.assertEqual(self.tasks(1005), {self.key: ('unread', 100, 'current')})
        ...
        self.root['freshness'] = 'uncertain'; self.root['restartUncertain'] = True
        self.root['notices'][1]['acknowledgedBy'].append('nanoleaf'); self.advance(1006)
        self.assertEqual(self.tasks(1006), {self.key: ('unread', 100, 'uncertain')})
        ...
        self.root['notices'][1]['acknowledgedBy'].remove('nanoleaf'); self.advance(1007, resync=True)
        self.assertEqual(self.tasks(1007), {self.key: ('unread', 100, 'current')})
        self.assertEqual(self.rows('SELECT started FROM activity'), epoch)
        self.assertEqual(self.rows('SELECT session FROM comets'), [])
