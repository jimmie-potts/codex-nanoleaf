...
export function admit(input) {
    const s = input.state;
    const reject = (decision) => ({ decision, reserved: false, nextSequence: s.nextSequence, scheduled: 0 });
    ...
    if (!validate('request', input.request))
        return reject('invalid-request');
    const r = input.request;
    const targetAuth = authorize({ ...input.auth, deviceId: r.deviceId, scope: 'control' });
    if (targetAuth.decision !== 'allowed')
        return reject(targetAuth.decision);
    if (r.controllerId !== s.controllerId || r.deviceId !== s.deviceId)
        return reject('unknown-device');
    if (!Number.isSafeInteger(input.bodyBytes) || input.bodyBytes < 0)
        return reject('invalid-request');
    if (input.bodyBytes > s.maxBodyBytes)
        return reject('capacity');
    if (r.requestId.epoch !== s.epoch)
    ...
    if (pending)
        return reject(sameJson(pending.request, r) ? 'join' : 'request-conflict');
    if (r.requestId.sequence < s.nextSequence)
        return reject('request-expired');
    if (r.requestId.sequence > s.nextSequence)
    ...
    let failure;
    if (r.expectedConfigurationRevision !== s.configurationRevision)
        failure = 'revision-conflict';
    else if (!sameTicket(r.expectedGeneration, s.generation))
        failure = 'stale-generation';
    else if (!supports(s.capabilities, r.command))
...
function batchAdmit(input) {
    ...
    for (const item of input.items) {
        ...
        if (result.reserved && result.receipt) {
            ...
            if (result.scheduled) {
                state.pending.push({ request: structuredClone(item.request) });
                state.inFlight += 1;
                state.queueDepth += 1;
            }
            else {
                state.cache.push({ request: structuredClone(item.request), receipt: result.receipt });
...
function clock(input) {
    ...
    if (!r || !validate('renderer', r))
        return { status: 'unknown' };
    if (!input.supportedProfiles.some(p => p.profileId === r.profileId && p.profileVersion === r.profileVersion))
    ...
    if (!input.fresh || r.clock.epoch !== input.expectedClockEpoch || r.deadlineMs === undefined
        ...
        || input.receivedAtLocalMs < 0 || input.nowLocalMs < input.receivedAtLocalMs)
        return { status: 'unknown' };
