...
triangles. It changes only the registered address, sends no light write, and a
running worker uses the new address from its next pass. It refuses `wall`, an
address another device uses and a device that does not match. `nanoleaf device-remove --device panels` removes a
