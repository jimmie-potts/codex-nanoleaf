...
- **Map edits.** A map edit to shared data (project colours, task projects, the palette) advances the Lines ledger, which owns shared configuration. Any other map edit advances the ledger of the device it names.
...
- **No migration.** Existing tables and rows are untouched. After a source rollback, older code keeps using the Lines ledger and ignores the Panels' tables, which wait for the newer source. Because an older revoke or disable misses those tables, the Panels' worker ends a queued request whose principal is inactive, or whose controller is disabled on the Lines ledger, without sending it. Review rejected an earlier design that added a `device` column: older code's positional writes then failed.
- **Concurrency.** Two worker instances now execute controller work in the same database. Each device's ledger is a separate set of tables, and each pass keeps the existing `BEGIN IMMEDIATE` transactions and the five-second busy timeout from ADR 0010.
