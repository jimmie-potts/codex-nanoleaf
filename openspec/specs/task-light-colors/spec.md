# task-light-colors Specification

## Purpose
Define which color each task state shows on every registered device, how the operator chooses those colors, and which colors stay fixed, so a finished task waiting for review stands out from unused Lines.
