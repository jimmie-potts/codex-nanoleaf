...
- Show the reservation select with a Reserve action, Swap halves and the task's project override only while Project layout is active. Classic shows none of them.
...
- `wall-line-identification`: the inspector identification requirement names the context card and limits reservation and half-swap controls to Project layout.
- `wall-task-inspector`: the thread-navigation and polling requirements name the context card and the Project-only override.

## Impact

`bridge/wall.html` markup, styles and the inspect, render, clearing and Options logic; the browser suites that clicked the header layout controls, Clear selection or the override in Classic; a new context-card browser check; the bridge guide; ADR 0012. No API, Python, state, database or device change. Draft PR #116 edits one line of the override handler in the same file; whichever merges second refreshes its comparison.
