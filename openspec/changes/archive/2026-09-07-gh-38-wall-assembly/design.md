...
- Structure from geometry. Endpoints within 40 units are joined into junctions. The hub is the junction joining the most Lines, and among ties the one with the smallest total distance to the other junctions (the bounding-box center alone picked the wrong four-way junction on the real wall). Every Line meeting the hub is a root at depth 0; a breadth-first walk assigns outward depth from there, and each unconnected section starts from its own nearest Line. The hinge of each Line is the endpoint nearer its parent (or the hub for roots). Alternative: animate by distance only; rejected because it ignores connectivity and would rotate Lines around the wrong end.
- Transforms on the existing groups. Each Line group animates from a folded rotation about its hinge (in user units via `transform-box: view-box`) and zero opacity to identity, using the Web Animations API with an `id` of `assembly`, staggered by depth. Nodes are not duplicated, so the final geometry is exactly what `drawWall()` drew. Connector nodes (a hexagon with an inner ring, like the physical connector and the tray icon) are permanent, drawn by `drawWall()` at every non-hub junction, and light with a flare as the first Line meeting them settles; the orb is a permanent circle trio at the hub.
