...
- **Codex home defaults.** `unread_reader` and `select_source` use `CODEX_HOME` or `~/.codex`, and `Metadata` reads only an explicit `metadata_path`; explicit `--desktop-state-path` and `--metadata-path` options keep pointing at mounted Windows JSON, which stays read-only.
