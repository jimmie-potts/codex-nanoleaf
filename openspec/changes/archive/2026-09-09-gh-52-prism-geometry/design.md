...
- Layout configuration has other owners. Keep its writes separate from the map-owned sidecar; the existing map-server singleton and App lock own connector enrichment. The sidecar envelope binds validated geometry to the layout file generation (mtime, size and file identity), so the existing remove-layout/rediscover procedure cannot reuse stale connectors when IDs remain unchanged. The in-memory cache carries the exact source generation accepted during enrichment, so a replacement between enrichment and App construction cannot label stale geometry as current. State reads compare the live file generation even when an in-memory graph exists. A changed file uses the same bounded acquisition budget and delay; successful refreshes replace connector and legacy zone drawing points together, while failures retain the prior graph. This private metadata is not projected to the browser. No worker state is rewritten.
