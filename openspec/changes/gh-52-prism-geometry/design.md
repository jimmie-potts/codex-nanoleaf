...
- Publish the validated additive cache with the bridge's existing atomic JSON writer before updating memory. The App lock serializes acquisition and state reads. Preserve every unrelated key in the latest saved layout. A failed write leaves memory and disk unchanged.
...
- Layout configuration has other owners. Read the latest saved layout immediately before the additive write and preserve its unrelated keys; the existing map-server singleton and App lock own this enrichment. No worker state is rewritten.
