...
for(const [name,status,body] of [['redirect',302,'{}'],['oversized',200,'x'.repeat(524289)],['malformed',200,'{'],['too deep',200,'['.repeat(34)+'0'+']'.repeat(34)]])test(`direct ${name} settles with possible effects after write`,async()=>{
 ...
 await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));
 try{await assert.rejects(exchange({transport:'windows-http',controllerPort:server.address().port,deviceId:'wall'},'command','a'.repeat(43),{}),error=>error.possible===true);assert.equal(count,1);}finally{server.closeAllConnections();await new Promise(resolve=>server.close(resolve));}
});
