...
for(const version of ['2025-11-25','2025-06-18'])test(`real MCP ${version} initialize, discovery, pure status and exact mode`,async t=>{
 ...
 const host=await startHost({enabled:true,port:0,controllerPort:external?Number(external):controller.address().port,controllerId:snapshot.identity.controllerId,deviceId:snapshot.identity.deviceId,transport:external?'wsl-helper':'windows-http',credentialsFile,...(external?{windowsPython:process.env.NANO_MCP_TEST_WINDOWS_PYTHON,windowsHelper:process.env.NANO_MCP_TEST_WINDOWS_HELPER}:{})});t.after(()=>host.close());
